...
- [x] Module `src/data_loader.py` is created with a `DataLoader` class.
- [x] `DataLoader.load_file_type(file_type: str) -> pd.DataFrame` returns a consolidated DataFrame with a `vintage` column.
- [x] `DataLoader.list_file_types() -> list[str]` returns all available file type identifiers.
- [x] `DataLoader.list_vintages(file_type: str) -> list[str]` returns vintages for a given file type.
- [x] Consolidated DataFrames are cached in memory (and optionally written to `data/consolidated/<file_type>.parquet`).
- [x] Unit tests in `tests/test_data_loader.py` cover:
